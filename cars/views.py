...
# --- ADD THIS TO THE BOTTOM OF cars/views.py ---
...
def fix_chat_db(request):
    """
    Manually creates the missing chat tables to bypass migration conflicts.
    """
    if not request.user.is_superuser:
        return HttpResponse("Unauthorized", status=403)

    sql_conversation = """
    CREATE TABLE IF NOT EXISTS cars_conversation (
        id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        created_at timestamp with time zone NOT NULL,
        updated_at timestamp with time zone NOT NULL,
        buyer_id integer NOT NULL REFERENCES auth_user(id) DEFERRABLE INITIALLY DEFERRED,
        car_id bigint NOT NULL REFERENCES cars_car(id) DEFERRABLE INITIALLY DEFERRED,
        dealer_id integer NOT NULL REFERENCES auth_user(id) DEFERRABLE INITIALLY DEFERRED,
        CONSTRAINT cars_conversation_car_id_buyer_id_uniq UNIQUE (car_id, buyer_id)
    );
    """

    sql_message = """
    CREATE TABLE IF NOT EXISTS cars_message (
        id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        content text NOT NULL,
        is_read boolean NOT NULL,
        timestamp timestamp with time zone NOT NULL,
        conversation_id bigint NOT NULL REFERENCES cars_conversation(id) DEFERRABLE INITIALLY DEFERRED,
    ...
    with connection.cursor() as cursor:
        try:
            ...
            return HttpResponse("<h1 style='color:green'>SUCCESS: Chat Tables Created!</h1> <p>You can now use the messaging system.</p>")
        except Exception as e:
